...
@RestController
@CrossOrigin
public class IbankAuthenticationController {
	...
	@RequestMapping(value = "/ibank/token", method = RequestMethod.POST)
	public ResponseEntity<?> createAuthenticationToken(@RequestBody ApiToken apiToken,
		...
		// 	if (jwtUser != null && !basicToken.equals(jwtUser.getBasicToken())) {
		// 		JwtErrorResponse jwtErrorResponse = new JwtErrorResponse(HttpStatus.BAD_REQUEST,
	...
	private boolean checkRequestIP(String username, String remoteAddr) {
		try {
			IbankUser ibankUser = ibankUserService.findByUserName(username);
			String allowIP = ibankUser.getWhitelistIp().trim();
